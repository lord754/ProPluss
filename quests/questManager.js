...
class QuestManager {
    ...
    static fromResponse(client, response) {
        const quests = (response.quests || []).map(q => Quest.create(q, false));
        const bounties = (response.bounties || []).map(q => Quest.create(q, true));
        return new QuestManager(client, [...quests, ...bounties]);
    }
    ...
    async runPlay(quest, taskConfig, taskName) {
        const appId = quest.config.application?.id || quest.config.id;
        const appName = quest.config.application?.name || quest.config.messages?.quest_name || 'Unknown';
