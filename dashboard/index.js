...
module.exports = (clientRef, clientsMap) => {
    ...
    app.get('/api/bot-ready', (req, res) => {
        ...
        // Ready if the active client is up, OR any client in the map is up
        const activeReady = !!getClient().user;
        const anyReady = clientsMap ? [...clientsMap.values()].some(c => !!c.user) : activeReady;
        const ready = activeReady || anyReady;
