...
class ServerCloner {
    ...
    async cloneServer(sourceGuildId, targetGuildId, options = {}) {
        try {
            ...
            // Defaults
            const opts = {
                deleteChannels: options.deleteChannels !== false,
                deleteRoles: options.deleteRoles !== false,
                deleteEmojis: options.deleteEmojis !== false,
                ...
                // channelMode: 'both' | 'channels_only' | 'categories_only' | 'separately'
                channelMode: options.channelMode || 'both'
            };
            ...
            if (opts.cloneChannels) {
                const m = opts.channelMode;
                if (m === 'both') {
                    await this.cloneCategories(sourceGuild, targetGuild);
                    await this.cloneChannels(sourceGuild, targetGuild, true);
                } else if (m === 'categories_only') {
                    await this.cloneCategories(sourceGuild, targetGuild);
                } else if (m === 'channels_only') {
                    await this.cloneChannels(sourceGuild, targetGuild, false);
                } else if (m === 'separately') {
                    await this.cloneCategories(sourceGuild, targetGuild);
                    await this.cloneChannels(sourceGuild, targetGuild, false);
                }
            }
    ...
    async cloneChannels(sourceGuild, targetGuild, withParent = true) {
        ...
        for (const [, channel] of channels) {
            ...
            try {
                ...
                let parent = null;
                if (withParent && channel.parent) {
                    parent = targetGuild.channels.cache.find(c => c.name === channel.parent.name && c.type === 'GUILD_CATEGORY') || null;
                }
